import './news.css';
import { TArticleValues } from '../../../interface/interface';

class News {
  public draw(data: TArticleValues[]): void {
    const news: TArticleValues[] = data.length >= 10 ? data.filter((_item, idx) => idx < 10) : data;

    const fragment: DocumentFragment = document.createDocumentFragment();
    const newsItemTemp = document.querySelector('#newsItemTemp') as HTMLTemplateElement;

    news.forEach((item: TArticleValues, idx: number) => {
      const newsClone = newsItemTemp.content.cloneNode(true) as HTMLElement | null;
      if (newsClone === null) throw new Error('No idea what went wrong.');
